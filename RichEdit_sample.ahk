...
global
	...
	; msgbox % "Window Handle " GuiID
	; msgbox % "RichEdit Handle " RE2.HWND
	OLECallbackVars()
...
If (A_GuiEvent = "N") {
	...
	Else If (Msg = 0x070B) { ; EN_LINK ;****ADDED DIGIDON
		  ...
		  ; tooltip WM_LBUTTONDBLCLK 
			 ...
			 ; msgbox % Instr(URLtoOpen,Chr(160))
			 URLtoOpen:=StrReplace(URLtoOpen,Chr(160),"%20")
			 URLtoOpen:=StrReplace(URLtoOpen,"%20"," ")
			 URLtoOpen:=StrReplace(URLtoOpen,"file://")
			
			 URLtoOpen:= """" URLtoOpen """"
			 try
			 {
			 tooltip % "Opening link " URLtoOpen
			 Run %URLtoOpen%
			 SetTimer( "RemoveToolTip", -1000)
			 }
			 catch error
				 {
				 TooltipAndRemove("Unable to open link " URLtoOpen,-3000)
				 }
			}
		}
	}
